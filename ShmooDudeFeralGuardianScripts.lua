# ShmooDude Feral and Guardian script - Modified by Attiqus
###
### Options:
...
#variable,name=RakePandemic,value=(15-0.20*15*talent.jagged_wounds.enabled)*0.3
AddFunction RakePandemic
{
    { 15 - 0.20 * 15 * TalentPoints(jagged_wounds_talent) } * 0.3
}
#variable,name=RipPandemic,value=(24-0.20*24*talent.jagged_wounds.enabled)*0.3
AddFunction RipPandemic
{
    { 24 - 0.20 * 24 * TalentPoints(jagged_wounds_talent) } * 0.3
}
#variable,name=ThrashCatPandemic,value=(15-0.20*15*talent.jagged_wounds.enabled)*0.3
...
#variable,name=PoolingValue,value=75
...
#variable,name=SavageRoar_Down,value=buff.savage_roar.down&(variable.FinisherConditions|time<12)
#savage_roar,if=variable.SavageRoar_Down
AddFunction SavageRoar_Down
{
    BuffExpires(savage_roar_buff) 
        and { FinisherConditions() or TimeInCombat() < 12 }
}
...
#variable,name=SavageRoar_Main,value=(buff.savage_roar.remains<=11|(buff.savage_roar.remains<variable.SavageRoarRefresh&target.health.pct>=variable.ExecuteRange))
#savage_roar,if=variable.SavageRoar_Main
AddFunction SavageRoar_Main
{
    BuffRemaining(savage_roar_buff) <= 11
...
AddFunction FeralPrecombatMainActions
{
    ...
    # MODIFICATION: Talent(bloodtalons_talent) to Talent(bloodtalons_talent) and BuffRemaining(bloodtalons_buff) < 20 and BuffExpires(prowl_buff)
    # REASON: Only suggest Regrowth out of stealth and if there's <20 seconds remaining
    if Talent(bloodtalons_talent) and BuffRemaining(bloodtalons_buff) < 20 and BuffExpires(prowl_buff) Spell(regrowth)
    #cat_form
    Spell(cat_form)
    #savage_roar
    if BuffRemaining(savage_roar_buff) < 6 + 6 * ComboPoints() Spell(savage_roar)
}
...
AddFunction FeralPrecombatShortCdActions
{
    ...
    unless Talent(bloodtalons_talent) and BuffRemaining(bloodtalons_buff) < 20 and BuffExpires(prowl_buff) and Spell(regrowth)
    {
        #prowl
        Spell(prowl)
    }
...
AddFunction FeralPrecombatShortCdPostConditions
{
    ...
    # MODIFICATION: Remove `or Spell(cat_form)`
    # REASON: Blocks Prowl from showing up
    Talent(bloodtalons_talent) and BuffRemaining(bloodtalons_buff) < 20 and BuffExpires(prowl_buff) and Spell(regrowth)
}
